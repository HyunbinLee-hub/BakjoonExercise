...
#define ARGMAX 3		// 명령행 인자의 최대 개수
#define BUFMAX 1024		// 버퍼의 크기
#define PATHMAX BUFMAX*2	// 경로명의 길이
#define REGFILE 1		// 정규파일
#define DIRECTORY 2		// 디렉토리 파일

typedef struct fileinfo {	// 파일정보
	char path[PATHMAX];
	size_t size;
	int type;
	int depth;
...
fileinfo origin;
int list_index = 1;		// 리스트 인덱스 번호
int is_dir_compare;
int r_option, i_option, q_option, s_option;
int block_num;
// o_: origin file, n_: new file
int o_line, n_line;
int o_common[BUFMAX], n_common[BUFMAX];			// 공통 문자열의 원본, 비교본에서의 라인 번호
int o_block[BUFMAX][2], n_block[BUFMAX][2];
char o_content[BUFMAX][BUFMAX], n_content[BUFMAX][BUFMAX];	// 원본과 비교본 파일의 내용

void command_help(void);	// help 명령어 실행
void command_find(int argc, char* argv[]);	// find 명령어 실행
// 디렉토리 차이 비교
...
void read_directory(int depth, char* dirname, fileinfo* filelist)
{
	...
	for (int i = 0; i < count; i++) {
		...
		else if (S_ISREG(statbuf.st_mode)) {	// 원본 파일과 이름, 크기 동일한 정규파일 발견
...
int compare_file(char* fname1, char* fname2)
{
	...
	fp[0] = fopen(fname1, "r");	// 원본 파일명
	fp[1] = fopen(fname2, "r");	// 비교본 파일명
